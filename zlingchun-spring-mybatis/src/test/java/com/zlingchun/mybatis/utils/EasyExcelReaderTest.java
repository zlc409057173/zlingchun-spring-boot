...
public class EasyExcelReaderTest {
    ...
    @Test
    void simpleRead() {
        // 写法1：JDK8+ ,不用额外写一个ReaderDataListener
        // since: 3.0.0-beta1
        ...
        // 写法2：
        // 匿名内部类 不用额外写一个ReaderDataListener
        ...
        // 有个很重要的点 ReaderDataListener 不能被spring管理，要每次读取excel都要new,然后里面用到spring可以构造方法传进去
        // 写法3：
        fileName = TestFileUtil.getPath() + "demo" +  File.separator + "demo.xlsx";
        // 这里 需要指定读用哪个class去读，然后读取第一个sheet 文件流会自动关闭
        EasyExcel.read(fileName, ReaderData.class, new ReaderDataListener(snowflake, dataService)).sheet().doRead();

        // 写法4
        fileName = TestFileUtil.getPath() + "demo" +  File.separator + "demo.xlsx";
        // 一个文件一个reader
        ExcelReader excelReader = EasyExcel.read(fileName, ReaderData.class, new ReaderDataListener(snowflake, dataService)).build();
     ...
     * <p>2. 由于默认一行行的读取excel，所以需要创建excel一行一行的回调监听器，参照{@link ReaderDataListener}
     * <p>3. 直接读即可
     */
    @Test
    ...
    public void repeatedRead() {
        String fileName = TestFileUtil.getPath() + "demo" + File.separator + "demo.xlsx";
        // 读取全部sheet
        // 这里需要注意 ReaderDataListener的doAfterAllAnalysed 会在每个sheet读取完毕后调用一次。然后所有sheet都会往同一个ReaderDataListener里面写
        EasyExcel.read(fileName, ReaderData.class, new ReaderDataListener(snowflake, dataService)).doReadAll();
        // 读取部分sheet
        fileName = TestFileUtil.getPath() + "demo" + File.separator + "demo.xlsx";
        ...
        ReadSheet readSheet1 =
                EasyExcel.readSheet(0).head(ReaderData.class).registerReadListener(new ReaderDataListener(snowflake, dataService)).build();
        ReadSheet readSheet2 =
                EasyExcel.readSheet(1).head(ReaderData.class).registerReadListener(new ReaderDataListener(snowflake, dataService)).build();
    ...
    public void converterRead() {
        ...
        EasyExcel.read(fileName, ReaderData.class, new ReaderDataListener(snowflake, dataService))
    ...
    public void complexHeaderRead() {
        String fileName = TestFileUtil.getPath() + "demo" + File.separator + "demo.xlsx";
        // 这里 需要指定读用哪个class去读，然后读取第一个sheet
        EasyExcel.read(fileName, ReaderData.class, new ReaderDataListener(snowflake, dataService)).sheet()
