...
public class EasyExcelWriterTest {
    ...
    public void excludeOrIncludeWrite() {
        String fileName = TestFileUtil.getPath() + "excludeWrite" + System.currentTimeMillis() + ".xlsx";
        ...
        fileName = TestFileUtil.getPath() + "includeWrite" + System.currentTimeMillis() + ".xlsx";
    ...
    public void repeatedWrite() {
        ...
        // 这里 需要指定写用哪个class去写
        ExcelWriter excelWriter1 = EasyExcel.write(fileName, DemoData.class).build();
        // 这里注意 如果同一个sheet只要创建一次
        WriteSheet writeSheet1 = EasyExcel.writerSheet("模板").build();
        // 去调用写入,这里我调用了五次，实际使用时根据数据库分页的总的页数来
        for (int i = 0; i < 5; i++) {
            // 分页去数据库查询数据 这里可以去数据库查询每一页的数据
            List<DemoData> data = data();
            excelWriter1.write(data, writeSheet1);
        }

        // 方法2: 如果写到不同的sheet 同一个对象
        fileName = TestFileUtil.getPath() + "repeatedWrite" + System.currentTimeMillis() + ".xlsx";
        // 这里 指定文件
        ExcelWriter excelWriter2 = EasyExcel.write(fileName, DemoData.class).build();
        ...
        fileName = TestFileUtil.getPath() + "repeatedWrite" + System.currentTimeMillis() + ".xlsx";
        // 这里 指定文件
        ExcelWriter excelWriter3 = EasyExcel.write(fileName).build();
        // 去调用写入,这里我调用了五次，实际使用时根据数据库分页的总的页数来。这里最终会写到5个sheet里面
        for (int i = 0; i < 5; i++) {
    ...
    public void imageWrite() throws Exception {
        ...
        String imagePath = TestFileUtil.getPath() + File.separator + "zz.jpg";
        try (InputStream inputStream = FileUtils.openInputStream(new File(imagePath))) {
            ...
            imageDemoData.setUrl(new URL(
                    "https://pics3.baidu.com/feed/94cad1c8a786c917659c8d76ee99e0c53ac75714.jpeg"));
    ...
    public void templateWrite() {
        String templateFileName = TestFileUtil.getPath() + File.separator + "demo.xlsx";
     ...
     * poi 自带{@link org.apache.poi.xssf.streaming.SXSSFSheet#autoSizeColumn(int)} 对中文支持也不太好。目前没找到很好的算法。 有的话可以推荐下。
