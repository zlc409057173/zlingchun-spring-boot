package com.zlingchun.mybatisplus.util.test;
...
public class RandomInfo {
    ...
    public static final int CHINA_MOBILE_OP = 0;
    public static final int CHINA_UNICOM_OP = 1;
    public static final int CHINA_TELECOME_OP = 2;
    //中国移动
    ...
    public static String createMobile(int op) {
        ...
        String mobile13;//手机号前三位
        int temp;
        switch (op) {
            case CHINA_MOBILE_OP:
                mobile13 = CHINA_MOBILE[randomInt(CHINA_MOBILE.length)];
                break;
            case CHINA_UNICOM_OP:
                mobile13 = CHINA_UNICOM[randomInt(CHINA_UNICOM.length)];
                break;
            case CHINA_TELECOME_OP:
                mobile13 = CHINA_TELECOME[randomInt(CHINA_TELECOME.length)];
                break;
            default:
                mobile13 = "op标志位有误！";
                break;
        }
        if (mobile13.length() > 3) {
            return mobile13;
        }
        sb.append(mobile13);
        //生成手机号后8位
    ...
    //名字中出现一个或者两个字的几率
    private static int pluralProbability = 5;
    ...
    public static String getRandomBoyName(){
        return getRandomName(boyName);
    }
    /**
     * 获取女生姓名
     */
    public static String getRandomGirlName(){
        return getRandomName(girlName);
    }
    /**
     * 传入一些名，随机取一个名字
     * @param name
    ...
    public static String getRandomName(String name){
        if(!StringUtils.hasText(name)){
            name = randomInt(2) == 0 ? boyName : girlName;
        }
        int nameIndexOne = randomInt(name.length());
        int nameIndexTwo = randomInt(name.length());
        if (randomInt(100) > surnameProbability){
            int familyOneNameIndex = randomInt(familyOneName.length());
            if(randomInt(10)>=pluralProbability){
                return familyOneName.substring(familyOneNameIndex, familyOneNameIndex+1) +
                        name.substring(nameIndexOne, nameIndexOne+1);
            }
            return familyOneName.substring(familyOneNameIndex, familyOneNameIndex+1) +
                    name.substring(nameIndexOne, nameIndexOne+1) +
                    name.substring(nameIndexTwo, nameIndexTwo+1);
        }else {
     ...
     * 获取随机生日日期
     */
    public static LocalDate getRandomBirthday(int year, int month, int week, int day){
        return LocalDate.now().minusYears(random.nextInt(year)).minusMonths(random.nextInt(month)).minusWeeks(random.nextInt(week)).minusDays(random.nextInt(day));
    }
    /**
     * 获取随机生日日期
     */
    public static LocalDate getRandomBirthday(){
        return getRandomBirthday(80, 12, 7, 30);
    }
